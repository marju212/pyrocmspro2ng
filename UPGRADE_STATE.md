...
27. **`.env`-driven environment + DB credentials.** PyroCMS 2.x predates composer, so we ship a tiny no-dependency dotenv loader at `system/cms/bootstrap/env.php` (~30-line parser, supports `KEY=value`, `# comments`, `'…'` / `"…"` quoting with `\n \r \t \" \\` escapes inside double quotes, leading `export ` for bash compatibility). The loader is required at the very top of `index.php` and populates `$_SERVER` / `$_ENV` / `getenv()` so any of CI's lookup conventions work — and **real environment variables always win** over `.env`, so staging / production can either keep a server-local `.env` or set vars via FPM `SetEnv` / docker `--env` / shell `export`. Two helpers exposed: `pyro_env($key, $default)` (string-or-fallback) and `pyro_env_bool($key, $default)` (bool coercion for `1/true/on/yes`). `index.php` reads `PYRO_ENV`, `APP_DISPLAY_ERRORS`, `APP_ERROR_LOG`, `APP_ERROR_REPORTING` (the last via a safe constant-name tokenizer — no `eval`); `system/cms/config/database.php` reads `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASS`, `DB_NAME`, `DB_DRIVER`, `DB_PREFIX`, `DB_CHARSET`, `DB_COLLATION`. All keys have hardcoded fallbacks so a fresh checkout boots without an `.env`; verified by temporarily moving `.env` aside and re-running the route baseline (46/46 pass) and the registration e2e (PASS). With debug knobs out of `index.php`, both `index.php` and `database.php` are now committed; `.gitignore` drops their entries and adds `.env` / `.env.local` / `.env.*.local`. The committed `.env.example` documents every supported key.
