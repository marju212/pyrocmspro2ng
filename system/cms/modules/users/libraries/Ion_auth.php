...
class Ion_auth
{
     ...
     * @author Jerel Unruh
     */
    public function activation_email($id = 0, $keep_active = false)
    {
        // Only deactivate if we're not keeping the user active (for welcome emails)
        if (!$keep_active) {
            $this->ci->ion_auth_model->deactivate($id);
        } else {
            // Generate activation code for email template but keep user active
            $activation_code = sha1(md5(microtime()));
            $this->ci->ion_auth_model->activation_code = $activation_code;
        }
