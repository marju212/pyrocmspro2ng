...
class Admin extends Admin_Controller
{
    ...
    public function edit($id = 0)
    {
        ...
        if ($this->form_validation->run() === true) {
            ...
            if ($update_data['active'] === '2') {
                Events::trigger('activation_email_sent', $id);
                $this->ion_auth->activation_email($id, true); // Keep user active when sending welcome email
                $update_data['active'] = 1; // Ensure user is activated
            } else {
    ...
    public function activate()
    {
        ...
        $to_activate = 0;
        foreach ($ids as $id) {
            if ($this->ion_auth->activation_email($id)) {
                // $this->ion_auth->activate($id)  if ($this->ion_auth->activation_email($id)) {
                $activated++;
