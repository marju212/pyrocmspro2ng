<?php echo form_open(current_url(), 'id="frm-builder" class="crud"'); ?>
<input type="hidden" name="frm_id" id="frm_id" value="<?php echo $form_id; ?>">
<div class="one_full">
  <section class="title">
    <ul>
      <li>
        <h4 class="handle"><?php echo lang('pf_build_title'); ?></h4>
      </li>
      <li>
        <a class="frm-save button btn blue tooltip" title="Save form" href="#">Save</a>
      </li>
      <li>
        <a class="frm-save frm-preview tooltip button btn orange" title="Preview form" href="#">Preview</a>
      </li>
      <li>
        <a class="button btn green tooltip" type="button" id="show_all" href="#" title="Expand all form element" style="display:none;">Show All</a>
      </li>
      <li>
        <a class="button btn green tooltip" id="hide_all" href="#" title="Collapse all form element">Hide All</a>
      </li>
      <li>
        <a class="add-new button tooltip" href="#" title="Add form element">Add field</a>
      </li>
    </ul>

  </section>
</div>

<div class="one_full">
  <div class="item fbuilder">
      <div class="form_inputs">
          <fieldset>
              <ul class="form-items stripe"></ul>
          </fieldset>
      </div>
  </div>
</div>
</form>

<script id="form-g-row" data-type="checkbox" type="text/x-jquery-tmpl">
    <li style="clear: both;display: block;width: 100%;">
        <span class="one_quarter"><input type="text" name="value" value="${value}" placeholder="<?php echo lang('pf_global:value') ?>" class="tooltip-s" title="<?php echo lang('pf_grp_value_title') ?>" /></span>
        <span class="one_quarter">

            <input type="text" name="label" value="${label}" placeholder="<?php echo lang('pf_global:label') ?>" class="tooltip-s" title="<?php echo lang('pf_grp_label_title') ?>" />
            <input type="checkbox" name="selected" value="1" class="tooltip-s" {{if typeof selected != 'undefined' && (selected === 'true' || selected === true) }}checked="checked"{{/if}} title="<?php echo lang('pf_grp_default_title') ?>" />
        </span>
        <span class="one_half">
            <input class="del button red" type="button" value="<?php echo lang('pf_global:remove') ?>" class="tooltip-s" title="<?php echo lang('pf_global:remove') ?>" />
        </span>
    </li>
</script>

<script id="form-groups" data-type="checkbox" type="text/x-jquery-tmpl">

    <li class="optbox p100">
        <label><?php echo lang('pf_grp_title') ?><span class="help-icon" data-help="form_group"></span>
        </label>
        <div class="grpwrap">
            <span class="p100">
                <input class="add button green tooltip-s" type="button" value="<?php echo lang('pf_global:add') ?>" class="tooltip-s" title="<?php echo lang('pf_grp_add_title') ?>" />
            </span>
            <br style="clear:both" />
            <ul class="items">
                {{if rows}}
                {{tmpl(rows) "#form-g-row"}}
                {{/if}}
            </ul>
        </select>
    </div>
</li>
</script>
<script id="form-def-value" type="text/x-jquery-tmpl">
  <li data-type="info" class="one_half default_value">
      <label><?php echo lang('pf_global:value') ?>
      <span class="help-icon" data-help="form_info"></span>
      </label>
        <input type="text" value="${fldDefault}" data-type="fldValue" class="frmValue inputs" name="forms[value]" type="text" title="Field value" />
  </li>
</script>


<script id="form-item" type="text/x-jquery-tmpl">

  <li class="form-item gd" fld-id="${fldID}">
  <div class="one_full">
    <section class="title">
      <ul>
        <li>
          <h4 class="handle" title="Type - Title"><i class="icon-reorder"></i> <span class="type">${fldType}</span> - <span class="name">${fldLabel}</span></h4>
        </li>

        <li>
          <a href="#" class="button btn green frmTog tooltip icon-chevron-up" title="Hide/Show"></a>
        </li>
        <li>
          <a href="#" class="button btn red frmDel icon-remove-sign tooltip" title="Remove" fld-id="${fldID}"></a>
        </li>
      </ul>
    </section>
  </div>
      <section class="item" style="display: block;">
      <div class="content">
        <ol>
            <li data-type="name" class="one_half">
                <label>
                    <?php echo lang('pf_global:name') ?> (no spaces) <span>*</span>
                    <span class="help-icon" data-help="form_name"></span>
                </label>
                <input data-type="fldName" class="frm-name inputs" type="text" name="forms[${fldID}][name]" value="${fldName}" title="Field Name" />
            </li>

            <li data-type="label" class="one_half">
                <label><?php echo lang('pf_global:label') ?> <span>*</span>
                <span class="help-icon" data-help="form_label"></span>
                </label>
                <input data-type="fldLabel" class="frmLabel inputs" name="forms[${fldID}][label]" type="text" value="${fldLabel}" title="Field Label" />
                <input data-type="fldLabelVisible" class="inputs tooltip-s" type="checkbox" value="1" {{if fldLabelVisible && fldLabelVisible == 1}}checked="checked"{{/if}} title="Show the label" />
            </li>

            <li data-type="type" class="one_half">
                <label><?php echo lang('pf_global:type') ?>
                <span class="help-icon" data-help="form_type"></span>
                </label>
                <select data-type="fldType" class="frm-type inputs" title="Field type">
                    <option value="text"{{if fldType == "text"}} selected="selected"{{/if}}>Text</option>
                    <option value="textarea"{{if fldType == "textarea"}} selected="selected"{{/if}}>TextArea</option>
                    <option value="dropdown"{{if fldType == "dropdown"}} selected="selected"{{/if}}>Dropdown</option>
                    <option value="checkbox"{{if fldType == "checkbox"}} selected="selected"{{/if}}>Checkbox</option>
                    <option value="radio"{{if fldType == "radio"}} selected="selected"{{/if}}>Radio</option>
                    <option value="file"{{if fldType == "file"}} selected="selected"{{/if}}>File</option>
                    <option value="captcha"{{if fldType == "captcha"}} selected="selected"{{/if}}>Captcha</option>
                    <option value="hidden"{{if fldType == "hidden"}} selected="selected"{{/if}}>Hidden</option>
                    <option value="submit"{{if fldType == "submit"}} selected="selected"{{/if}}>Submit</option>
                </select>
            </li>
            <li data-type="info" class="one_half">
                <label><?php echo lang('pf_global:info') ?>
                <span class="help-icon" data-help="form_info"></span>
                </label>
                <textarea data-type="fldInfo" class="frmInfo inputs" name="forms[${fldID}][info]" type="text" title="Field Info">${fldInfo}</textarea>
            </li>


            <li data-type="valid" class="one_half valid"{{if fldType == "button" || fldType == "submit" || fldType == "captcha" }} style="display:none;"{{/if}}>
                <label><?php echo lang('pf_field_validation_title') ?>
                <span class="help-icon" data-help="form_validation"></span>
                </label>
                <div>
                    <span class="tooltip-s" title="<?php echo lang('pf_required_field_title') ?>"><input data-type="fldValidReq" class="frmValidreq inputs" name="forms[${fldID}][validreq]" type="checkbox" value="1" {{if fldValidReq}}checked="checked"{{/if}} /> Required</span> &nbsp;&nbsp;
                    {{if fldType == 'checkbox'}}
                      <span class="tooltip-s" title="<?php echo lang('pf_min_length_title') ?>"> <input data-type="fldValidMinLen" class="frmValidminlen inputs tooltip-s" name="forms[${fldID}][validminlen]" type="input" size="3" value="${fldValidMinLen}" /> Min</span> &nbsp;&nbsp;
                      <span class="tooltip-s" title="<?php echo lang('pf_max_length_title') ?>"> <input data-type="fldValidMaxLen" class="frmValidmaxlen inputs tooltip-s" name="forms[${fldID}][validmaxlen]" type="input" size="3" value="${fldValidMaxLen}" /> Max</span> &nbsp;&nbsp;
                      {{/if}}

                    <span class="extra_valid" {{if this.hasExtras(fldType) == false || this.hasReq(fldType) }} style="display:none;"{{/if}}>
                      <span class="tooltip-s" title="<?php echo lang('pf_valid_email_title') ?>"><input data-type="fldValidEmail" class="frmValidemail inputs tooltip-s" name="forms[${fldID}][validemail]" type="checkbox" value="1" {{if fldValidEmail}}checked="checked"{{/if}} /> Email</span> &nbsp;&nbsp;
                      <span class="tooltip-s" title="<?php echo lang('pf_numeric_only_title') ?>"><input data-type="fldValidNumeric" class="frmValidnumeric inputs tooltip-s" name="forms[${fldID}][validemail]" type="checkbox" value="1" {{if frmValidNumeric}}checked="checked"{{/if}} /> Numeric</span> &nbsp;&nbsp;
                      <span class="tooltip-s" title="<?php echo lang('pf_alpha_only_title') ?>"> <input data-type="fldValidAlpha" class="frmValidalpha inputs tooltip-s" name="forms[${fldID}][validalpha]" type="checkbox" value="1" {{if frmValidAlpha}}checked="checked"{{/if}} /> Alpha</span> &nbsp;&nbsp;
                      <span class="tooltip-s" title="<?php echo lang('pf_alphadash_only_title') ?>"><input data-type="fldValidAlphaDash" class="frmValidalphadash inputs tooltip-s" name="forms[${fldID}][validalphadash]" type="checkbox" value="1" {{if ValidAlphaDash}}checked="checked"{{/if}} /> Alpha-Dash</span> &nbsp;&nbsp;
                      <span class="tooltip-s" title="<?php echo lang('pf_min_length_title') ?>"> <input data-type="fldValidMinLen" class="frmValidminlen inputs tooltip-s" name="forms[${fldID}][validminlen]" type="input" size="3" value="${fldValidMinLen}" /> Min</span> &nbsp;&nbsp;
                      <span class="tooltip-s" title="<?php echo lang('pf_max_length_title') ?>"> <input data-type="fldValidMaxLen" class="frmValidmaxlen inputs tooltip-s" name="forms[${fldID}][validmaxlen]" type="input" size="3" value="${fldValidMaxLen}" /> Max</span> &nbsp;&nbsp;
                    </span>
                </div>

            </li>

            <li data-type="type" class="one_half prep_data"{{if this.hasExtras(fldType) == false }} style="display:none;"{{/if}}>
                <label><?php echo lang('pf_prep_data_title') ?>
                <span class="help-icon" data-help="form_prep"></span>
                </label>
                <select data-type="fldPrep" class="frm-prep inputs"  multiple="true">
                    <option value="trim"{{if jQuery.inArray("trim", fldPrep) != -1}} selected="selected"{{/if}}><?php echo lang('pf_trim_label') ?></option>
                    <option value="xss"{{if jQuery.inArray("xss", fldPrep) != -1}} selected="selected"{{/if}}><?php echo lang('pf_xssclean_label') ?></option>
                    <option value="url"{{if jQuery.inArray("url", fldPrep) != -1}} selected="selected"{{/if}}>URL</option>
                </select>
            </li>

            {{if fldType == "dropdown" || fldType == "checkbox" || fldType == "radio" }}
              {{tmpl "#form-groups"}}
            {{/if}}
            
            {{if fldType == "text" || fldType == "textarea" || fldType == "hidden" }}
              {{tmpl "#form-def-value"}}
            {{/if}}
        </ol>
      </div>
    </section>

    </li>

</script>



<script type="text/javascript" src="http://ajax.microsoft.com/ajax/jquery.templates/beta1/jquery.tmpl.js"></script>
<script type="text/javascript">
var modPath = '<?php echo $moduri; ?>';
var PF_Global = {
    t: {
        dup_fld_title: '<?php echo lang('pf_duplicate_field_names_title');?>',
        dup_fld_notice: '<?php echo lang('pf_duplicate_field_names_notice');?>',
        insert_success: '<?php echo lang('pf_insert_success');?>'
    }
};
(function(c){function b(a){this.options=c.extend({},b.defaults,a)}b.instances=[];b.repositionAll=function(){for(var a=0;a<b.instances.length;a++)if(b.instances[a]){var e=b.instances[a].options;(new b(e)).positionImg(c(b.instances[a].target),c.data(b.instances[a].target,"busy"),e.position)}};b.prototype.hide=function(a){a.each(function(){var a=c.data(this,"busy");a&&a.remove();c(this).css("visibility","");c.data(this,"busy",null);for(a=0;a<b.instances.length;a++)null!=b.instances[a]&&b.instances[a].target==
this&&(b.instances[a]=null)})};b.prototype.show=function(a){var e=this;a.each(function(){if(!c.data(this,"busy")){var a=c(this),d=e.buildImg();d.css("visibility","hidden");d.load(function(){e.positionImg(a,d,e.options.position);d.css("visibility","")});c("body").append(d);e.options.hide&&a.css("visibility","hidden");c.data(this,"busy",d);b.instances.push({target:this,options:e.options})}})};b.prototype.preload=function(){var a=this.buildImg();a.css("visibility","hidden");a.load(function(){c(this).remove()});
c("body").append(a)};b.prototype.buildImg=function(){var a="<img src='"+this.options.img+"' alt='"+this.options.alt+"' title='"+this.options.title+"'";this.options.width&&(a+=" width='"+this.options.width+"'");this.options.height&&(a+=" height='"+this.options.height+"'");return c(a+" />")};b.prototype.positionImg=function(a,b,c){var d=a.offset(),f=a.outerWidth(),a=a.outerHeight(),g=b.outerWidth(),h=b.outerHeight(),c="left"==c?d.left-g-this.options.offset:"right"==c?d.left+f+this.options.offset:d.left+
(f-g)/2,d=d.top+(a-h)/2;b.css("position","absolute");b.css("left",c+"px");b.css("top",d+"px")};b.defaults={img: modPath + "img/busy.gif",alt:"Please wait...",title:"Please wait...",hide:!0,position:"center",zIndex:1001,width:null,height:null,offset:10};c.fn.busy=function(a,e){if(-1!=c.inArray(a,["clear","hide","remove"]))(new b(a)).hide(c(this));else if("defaults"==a)c.extend(b.defaults,e||{});else if("preload"==a)(new b(a)).preload();else if("reposition"==a)b.repositionAll();
else return(new b(a)).show(c(this)),c(this)}})(jQuery);
jQuery(function($) {
  var pf_help_data = {form_name:"Name of the field (no spaces).", form_label:"Labels for the field.", form_type:"What type of field is this?", form_info:"Information about the field.", form_prep:'Would you like to format the data in any way?<br><ul><li><b><u>Trim</u></b>&nbsp; trim spaces off the beginning and end of the data.</li><li><b><u>XSS Clean</u></b>&nbsp; clean and filter input to prevent cross-site scripting attacks.</li><li><b><u>URL</u></b>&nbsp; Adds "http://" to URLs if missing.</li></ul>', 
  form_validation:"Rules used to validate the field.", form_group:"Use the interface below to create a field group."};
  $(".fbuilder .help-icon").tipsy({html:!0, gravity:"w", live:!0, title:function() {
    return pf_help_data[$(this).attr("data-help")]
  }});
  var _busy = !1;
  $.fn.myEdit = function() {
    $(this);
    this.each(function() {
      var a = this, b = $("ul.items", this), c = $(".val", a);
      oLab = $(".lab", a);
      $(".lab, .val", a).keydown(function(b) {
        "13" == b.keyCode && (b.preventDefault(), $(".add", a).trigger("click"))
      });
      $(".add", this).live("click", function() {
        $("#form-g-row").tmpl().hide().appendTo(b).fadeIn("slow");
        oLab.val("");
        c.val("").focus()
      });
      $(".del", this).live("click", function() {
        $(this).closest("li").animate({opacity:"hide", height:"hide", marginBottom:"0px"}, "fast", function() {
          $(this).remove()
        })
      });
      $(".edt", this).live("click", function() {
        var a = $(":selected", b);
        oLab.val(a.text());
        c.val(a.val())
      })
    })
  };
  var tmplMethods = {
    noExtrasFlds: "button,submit,reset,clear,captcha,hidden,paragraph".split(","),
    onlyRequired:"file,checkbox,radio,dropdown".split(","),
    hasGroup:function(a) { return-1 < $.inArray(a, ["dropdown", "checkbox", "radio"]) ? !0 : !1},
    hasExtras:function(a) {return -1 < $.inArray(a, this.noExtrasFlds) || this.hasReq(a) == true ? !1 : !0},
    hasReq:function(a) {return-1 < $.inArray(a, this.onlyRequired) ? !0 : !1}
  };


  $("#frm-builder .frm-save").click(function(a) {
    a.preventDefault();
    _busy = $(".add-new").busy({position:"left", hide:!1});
    var b = {}, c = $(this), e = !1, d = !1;
    $(".frm-name").each(function() {
      var a = $(this);
      b[a.val()] ? (e = !0, !1 === d && (b[a.val()].focus(), d = !0), b[a.val()].css({"border-color":"#c00000"}), a.css({"border-color":"#c00000"})) : b[a.val()] = a
    });
    if(e) {
      return Notifier.error(PF_Global.t.dup_fld_notice, PF_Global.t.dup_fld_title), _busy.busy("hide"), !1
    }
    var f = [];

/**
 * @TODO Fix duplicate fields bug
 * Fields get duplicated because the data-id does not get updated or set...
 */
    $(".form-item", $("#frm-builder")).each(function() {
      var a = {}, b = {}, c = [];
      a.fldId = $(this).attr("fld-id")||0;

      $("ol li", this).each(function() {
        var d = $(".grpwrap", this);
        //Loop group items (dropdown, checkbox, radio)
        d && $("ul.items", d).each(function() {
          var c = [];
          $(this).children("li").each(function() {
            b = {};
            $(":input", this).each(function() {
              var a = $(this), c = $(this).attr("name");
              if(c == void 0) {
                return true
              }
              c == "selected" ? b[c] = a.is(":checked") : c != "rem" && (b[c] = a.val())
            });
            c.push(b)
          });
          a.data = c
        });

        //Loop meta fields
        $(this).find(".inputs").each(function() {
          var b = $(this).val(), d = $(this).attr("type"), e = $(this).attr("data-type");
          e && (e.substr(0, 8) === "fldValid" ? d == "checkbox" ? $(this).is(":checked") && c.push(e) : d == "input" && b.length > 0 && c.push({type:e, val:b}) : d == "checkbox" ? a[$(this).attr("data-type")] = $(this).is(":checked") : a[$(this).attr("data-type")] = b)
        })
      });
      0 < c.length && (a.fldValidation = c);
      f.push(a)
    });
    0 < f.length && $.ajax({type:"POST", url:$("#frm-builder").attr("action"), data:{fields:f}, success:function(a) {
      if(0 < a.ids.length) {
        var b = 0;
        $(".form-item").each(function() {
          if ($(this).attr("fld-id") == 0) {$(this).attr("fld-id", a.ids[b]); b += 1;}
        })
      }
      _busy.busy("hide");
      Notifier.success(PF_Global.t.insert_success);
      c.hasClass("frm-preview") && (Notifier.info("Loading Preview.."), $.colorbox({height:"100%", width:"100%", iframe:!0, href:SITE_URL + "pyroforms/preview/" + $("#frm_id").val()}))
    }, dataType:"json"});
    return!1
  });

/**
 * Field:SORTABLE
 */
  $(".form-items").sortable({opacity:0.6, handle:".handle", axis: "y", cursor:"move"});

/**
 * Tooltips
 */
  $(".tooltip-s, .tooltip").livequery(function() {
    $(this).tipsy({gravity:"s", fade:!0, html:!0})
  });

/**
 * Field:ADD_NEW
 */
  $(".add-new").click(function(e) {
    e.preventDefault();
    var a = {
      fldID:0,
      fldName: "fld-" + $(".form-item").length,
      frmLabel:"", fldLabelVisible:1, fldType:"text", fldPrep:["trim"], fldValidReq:0,
      frmValidTrim:1, frmValidClean:1, frmValidUrl:0, frmValidEmail:0, frmValidAlpha:0,
      frmValidAlphaDash:0, frmValidNumeric:0, frmValidMinLen:"", frmValidMaxLen:""
    };

    //Append field
    $("#form-item").tmpl(a, tmplMethods).hide().appendTo(".form-items").fadeIn("slow").find(".frmLabel").focus();
  });
  var _curType = "";
  $(".frm-type").chosen().live("focus", function() {
    _curType = $(this).val()
  });
  var gtoJSON = function(a) {
    var b = [];
    $("ul.items", a).each(function() {
      $(this).children("li").each(function() {
        var a = {};
        $(":input", this).each(function() {
          var b = $(this), d = $(this).attr("name");
          if(void 0 == d) {
            return!0
          }
          "selected" == d ? a[d] = b.is(":checked") : "rem" != d && (a[d] = b.val())
        });
        b.push(a)
      })
    });
    return b
  }, _oData = {};
  
/**
 * Field header:UPDATE Label text
 */
  $(".frmLabel").live("keyup", function() {
    var a = $(this).parents("li.form-item");
    $(".title .name", a).text($(this).val())
  });

/**
 * Field toggle :HIDE/SHOW
 */
  $(".frmTog").live("click", function(e) {
    e.preventDefault();
    var o = $(this),
        a = o.parents("li.form-item"),
        s = $("section.item", a);
    
    if (!s.is(":hidden"))
    {
      s.slideUp();
      o.removeClass("icon-chevron-up").addClass("icon-chevron-down")
    }
    else {
      s.slideDown();
      o.removeClass("icon-chevron-down").addClass("icon-chevron-up")
    }
  });

/**
 * Fields:HIDE_ALL
 */
  $("#hide_all").live("click", function(e) {
    e.preventDefault();
    $("#hide_all").hide();
    $("#show_all").show();
    $('.form-item > section.item').slideUp();
    $('.form-item .frmTog').removeClass("icon-chevron-up").addClass("icon-chevron-down");

    return false;
  });

/**
 * Fields:SHOW_ALL
 */
  $("#show_all").live("click", function(e) {
    e.preventDefault();
    $("#show_all").hide();
    $("#hide_all").show();
    $('.form-item > section.item').slideDown();
    $('.form-item .frmTog').removeClass("icon-chevron-down").addClass("icon-chevron-up");
    return false;
  });

/**
 * Field:DELETE
 */
  $(".frmDel").live("click", function(e) {
    e.preventDefault();
    if(confirm("Are you sure you want to remove this form item ?")) {
      var a = $(this).attr("fld-id"), b = $(this).parents("li.form-item").hide();
      "" == a ? b.remove() : $.ajax({type:"POST", url:BASE_URL + "admin/pyroforms/remove/" + a, success:function() {
        b.remove();
        $(".stripe > li").removeClass("alt").filter(":odd").addClass("alt")
      }, dataType:"json"})
    }
  });

/**
 * Field type:CHANGE
 */

  $('.frm-type').live('change',function(e) {

      var self=$(this), _fType = $(this).val();

      var parent = $(this).parents('li.form-item ol');
      var bParent = $(this).parents('li.form-item');
      var wObj = $('.optbox', parent);
      var vObj = $('.default_value', parent);

      $('.title .type',bParent).text(_fType);

      if (vObj.is(':visible')) {
          self.data('vData',{fldDefault: $('.frmValue', vObj).val()});
          vObj.remove();
      }
      if (wObj.is(':visible')) {
          self.data('oData',gtoJSON(wObj));
          wObj.remove();
      }


      if (tmplMethods.hasExtras(_fType))
      {
          $('.prep_data', parent).fadeIn();
          $('.valid', parent).fadeIn();
          $('.extra_valid', parent).fadeIn();
      }
      else {
        if (tmplMethods.hasReq(_fType))
        {
          $('.prep_data', parent).fadeOut();
          $('.extra_valid', parent).fadeOut();
          $('.valid', parent).fadeIn();
        }
        else {
            $('.prep_data', parent).fadeOut();
            $('.valid', parent).fadeOut();
            //vObj.fadeOut();
        }
      }
      if (tmplMethods.hasGroup(_fType)) {
          $('.default_value', bParent).fadeOut();
          $('#form-groups').tmpl({rows: self.data('oData')}).hide().appendTo(parent).fadeIn();
          $('.grpwrap').myEdit({});
          //if (_fType == 'checkbox' || _fType == 'file' || _fType == 'radio') {$('.valid', parent).fadeIn();}
          //$('.extra_valid', parent).fadeOut();
      }else { if (_fType != 'file') $('#form-def-value').tmpl(self.data('vData')).hide().appendTo(parent).fadeIn();}

      $('.tooltip-s').tipsy({gravity: 's',fade: true,html: true});
  });

/**
 * Notify Scripts
 */
  var config = window.NotifierjsConfig = {defaultTimeOut:1000, position:["top", "right"], notificationStyles:{padding:"12px 18px", margin:"0 0 6px 0", backgroundColor:"#000", opacity:0.8, color:"#fff", font:"normal 13px 'Droid Sans', sans-serif", borderRadius:"3px", boxShadow:"#999 0 0 12px", width:"300px"}, notificationStylesHover:{opacity:1, boxShadow:"#000 0 0 12px"}, container:$("<div></div>")};
  $(document).ready(function() {
    config.container.css("position", "fixed");
    config.container.css("z-index", 9999);
    config.container.css(config.position[0], "12px");
    config.container.css(config.position[1], "12px");
    $("body").append(config.container)
  });
  function getNotificationElement() {
    return $("<div>").css(config.notificationStyles).hover(function() {
      $(this).css(config.notificationStylesHover)
    }, function() {
      $(this).css(config.notificationStyles)
    })
  }
  var Notifier = window.Notifier = {};
  Notifier.notify = function(a, b, c, e) {
    var d = getNotificationElement(), e = e || config.defaultTimeOut;
    c && (c = $("<img/>", {src:c, css:{width:36, height:36, display:"inline-block", verticalAlign:"middle"}}), d.append(c));
    c = $("<div/>").css({display:"inline-block", verticalAlign:"middle", padding:"0 12px"});
    if(b) {
      var f = $("<div/>");
      f.append(document.createTextNode(b));
      f.css("font-weight", "bold");
      c.append(f)
    }
    a && (b = $("<div/>"), b.append(document.createTextNode(a)), c.append(b));
    d.delay(e).fadeOut(function() {
      d.remove()
    });
    d.bind("click", function() {
      d.hide()
    });
    d.append(c);
    config.container.prepend(d)
  };
  Notifier.info = function(a, b) {
    Notifier.notify(a, b, "data:image/png;base64,%3D%3D")
  };
  Notifier.warning = function(a, b) {
    Notifier.notify(a, b, "data:image/png;base64,%3D")
  };
  Notifier.error = function(a, b) {
    Notifier.notify(a, b, "data:image/png;base64,%3D%3D")
  };
  Notifier.success = function(a, b) {
    Notifier.notify(a, b, "data:image/png;base64,%3D")
  };
  var message = $("section.title"), originalMessageTop = message.offset().top, view = $(window);
  view.bind("scroll resize", function() {
    var a = view.scrollTop();
    a > originalMessageTop && !message.is(".toolbar-fixed") ? message.addClass("toolbar-fixed") : a <= originalMessageTop && message.is(".toolbar-fixed") && message.removeClass("toolbar-fixed")
  });
  $(".stripe > li").removeClass("alt").filter(":odd").addClass("alt")
<?php
    if (!empty($frm)) {
        echo '$( "#form-item" ).tmpl(' . json_encode($frm) . ',tmplMethods).hide().appendTo( ".form-items" ).fadeIn("slow"); $(".grpwrap").myEdit({});';
    }
?>

    $(".stripe > li").removeClass("alt").filter(":odd").addClass("alt");
});


</script>