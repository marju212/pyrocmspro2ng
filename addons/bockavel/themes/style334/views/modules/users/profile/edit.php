<div class="panel article">
    <div class="panel-body">
        <h1><?php echo ($this->current_user->id !== $_user->id) ?
                sprintf(lang('user:edit_title'), $_user->display_name) :
                lang('profile_edit') ?></h1>
    </div>
    <div class="panel-body">
                ...
                <?php foreach ($profile_fields as $field): ?>
                    <?php if ($field['input']): ?>
                        <div class="form-group">
                            ...
                            <?php echo str_replace(array("<input", "<select", "<textarea"), array("<input class=\"form-control\"", "<select class=\"form-control\"", "<textarea class=\"form-control\""), $field['input']) ?>
                        </div>
                    <?php endif ?>
                <?php endforeach ?>
                <div class="checkbox" style="margin:30px 0;">
                    <label class="control-label" style="font-weight:bold;">
                        <input name="cae[]" value="1" class="checkbox" type="checkbox"> Min besättning är CAE
                        fri.
                    </label>
                </div>
            </div>
        ...
        <?php endif ?>
        <input name="medlemsvillkor[]" value="1" class="checkbox" type="hidden">
...
<script>
    $(function () {
        $('input[name="medlemsvillkor[]"].form-control').closest('.form-group').hide();
        $('input[name="cae[]"].form-control').closest('.form-group').hide();
    });
</script>
